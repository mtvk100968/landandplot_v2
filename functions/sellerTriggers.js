...
exports.onSellerUpdate = functions
  ...
  .onUpdate(async (change, ctx) => {
    ...
    if (addedAgents.length) {
      const message = `An agent was assigned to your listing.`;
      await createNotif(ownerId, 'agentAssigned', message);
    }
    ...
    if (newBuyers.length > oldBuyers.length) {
      ...
      const message = `New interest from ${latest.name}.`;
      await createNotif(ownerId, 'newInterest', message);
    }

    // 3) Buyer status change (visited → negotiating → accepted/rejected/bought)
    for (let i = 0; i < newBuyers.length; i++) {
      const o = oldBuyers[i] || {};
      const n = newBuyers[i];
      if (o.status !== n.status) {
        const message = `Buyer ${n.name} is now "${n.status}".`;
        await createNotif(ownerId, 'saleStage', message);
      }
    }
